...
function draw() {
  ...
  if (gameState !== "end") {
    ...
    monkey.velocityX = cam.velocityX;

    monkey.velocityY = monkey.velocityY + 3;

    Ground.x = cam.body.position.x;0

    if (cam.body.position.x % 150 === 0) {
      Background.x = cam.body.position.x;
    }

    Ground.x = cam.body.position.x;
...
function Bananas() {
  ...
  if (frameCount % 120 === 0) {
    var banana = createSprite(cam.body.position.x + 300);
...
function obstacles() {
  ...
  if (frameCount % 80 === 0) {
    var obstacle = createSprite(cam.body.position.x + 250, 200);
...
function endState() {
  ...
  gameOver = createSprite(cam.body.position.x, 120);
  gameOver.addImage("gameOVer", gameOverImage);
  restart = createSprite(cam.body.position.x, 200);
